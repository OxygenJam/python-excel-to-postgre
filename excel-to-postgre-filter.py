...
# Loads the sheet where the list of filtered columns
def loadSheetOfFilteredColumns(file_name):

    filters = []

    with open("../Data/Filters/" + file_name  + ".txt") as file:

        data = file.readlines()

        for line in data:

            column = formatColumnNames(line.strip())

            filters.append(column)

    #book = xlrd.open_workbook("../Data/Filters" + file_name)
    #sheet = book.sheet_by_index(0)
    
    # List of columns from excel file B in store in 1st column
    # To be used as filter for excel file A
    #for r in range(0,len(sheet.nrows)):

    #    column = str(sheet.cell(r,0).value)
...
def main():
    ...
    if(isSyntaxCorrect(args[1:])==-1):
        ...
        rPrint("e.g.\n")
        rPrint("python table-generator.py <some-file> <some-sheet> <table-name> <filter-filename> <where-clause>")
        ...
        logPrint("")
        args.append(str(input("Please provide the filename (.txt) of the column filter to be used: ")))

        logPrint("")
        args.append(str(input("Please provide the where clause (col_name = val): ")))
    ...
    filters = str(args[4])
    filters = loadSheetOfFilteredColumns(filters)

    whereClause = str(args[5])

    if(whereClause.find(" = ") == -1):
        whereClause = whereClause.replace("=", " = ")

    whereClause = whereClause.split(" = ")
    whereClause[0] = formatColumnNames(whereClause[0])

    logPrint(f"WHERE {whereClause[0]} == {whereClause[1]}\n")
    logPrint("Filters: \n\n ")
    print(colored(str(filters) + "\n", "green"))
    ...
    for c in range(0, sheet.ncols):
        col = sheet.cell(0,c).value
        col = formatColumnNames(col)

        if(col == whereClause[0]):
    ...
    with db.cursor() as cursor:
        ...
        for r in range(1,sheet.nrows):
            ...
            # If column is the where clause
            # and value is equal to where value
            if(str(sheet.cell(r,whereClause[2]).value) != whereClause[1]):
                continue
    ...
    logPrint(str(len(filters) + 2) + " number of columns / number of data per row\n\n")
